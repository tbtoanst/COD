...
import { TransactionReportService, FccService } from '@app/_services';
import { NotificationService } from '@app/_components/notification/_services/notification.service';
import { PageEvent } from '@angular/material/paginator';
import { TRANSFER_BUY_STATUS_LIST } from '@app/_constant/index';

@Component({
  selector: 'app-trans-transfer-not-approved',
  templateUrl: './trans-transfer-not-approved.component.html',
  styleUrls: ['./trans-transfer-not-approved.component.scss']
})
export class TransTransferNotApprovedComponent implements OnInit {

  @Input() isBlock:TransTransferNotApprovedComponent;
  ...
  private async _getList() {
    try {
      this.transferListSell = [];
      this.spinner.show();
      const res: any = await this._transactionReportService.cleanSellQuery(this._getFormRequest());
      if (res.data.result.length > 0) {
        this.transferListSell = res.data.result;
